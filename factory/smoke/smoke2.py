"""第二组无头冒烟（harness）：按游戏工序表通用驾驭。用法: smoke2.py <game> [out_dir] [task]"""
...
out = Path(sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] else str(ROOT / "reports" / "smoke" / f"shots2-{theme}")); out.mkdir(parents=True, exist_ok=True)
...
with sync_playwright() as p:
    ...
    t0 = time.time(); pg.goto(f"http://127.0.0.1:{port}/drinks/game-drink2/index.html?harness=1&game={theme}{rq}")
